...
namespace RedbonePlatform.Pages.ContainerPages
{
    public partial class PersonEdit_Personal : System.Web.UI.Page
    {
        ...
        protected void RadGrid_Certifications_ItemCommand(object sender, GridCommandEventArgs e)
        {
            string PersonId = HiddenField_PersonId.Value;
            string CertificationTypeId = "";
            string CertificationDescription = "";

            RadDropDownList dropDownListCertificationType = e.Item.FindControl("RadDropDownList_CertificationTypes") as RadDropDownList;
            if (dropDownListCertificationType != null && dropDownListCertificationType.SelectedIndex > -1)
            {
                CertificationTypeId = dropDownListCertificationType.SelectedItem.Value;
                CertificationDescription = dropDownListCertificationType.SelectedText;
                HiddenField_CertificationTypeId.Value = CertificationTypeId;
            }
            ...
            if (e.CommandName == "PerformInsert" || e.CommandName == "Update")
            {
                ...
                {
                    string filePath = "";
                    bool newORUpdatedfilePresent = false;
                    byte[] fileData = new byte[] { };

                    //File
                    GridEditableItem updateItem = e.Item as GridEditableItem;
                    RadAsyncUpload radAsyncUpload = updateItem.FindControl("RadAsyncUploadCert") as RadAsyncUpload;
                    RadLabel fileNameLbl = (RadLabel)e.Item.FindControl("FileNameLbl");
                    String certFileName = fileNameLbl.Text;

                    if (radAsyncUpload.UploadedFiles.Count > 0)
                    {
                        newORUpdatedfilePresent = true;
                    ...
                    if (newORUpdatedfilePresent && fileData != null)
                    {
                        filePath = AppDomain.CurrentDomain.BaseDirectory + "UserFiles\\" + PersonId.ToString() + "\\Qualification\\Certifications\\" + CertificationDescription + "\\" + radAsyncUpload.UploadedFiles[0].FileName;
                        try
                        {
        ...
        protected void RadGrid_Certifications_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            {
                RadDropDownList dropDownListCertificationType = e.Item.FindControl("RadDropDownList_CertificationTypes") as RadDropDownList;
                if (dropDownListCertificationType != null)
                {
                    dropDownListCertificationType.Enabled = true;
                }
            }

            //set file name
            RadLabel fileNameLbl = (RadLabel)e.Item.FindControl("FileNameLbl");
            if (fileNameLbl != null && fileNameLbl.Text != null && fileNameLbl.Text.Length > 0)
            {
                RadLabel fileNameOnlyLbl = (RadLabel)e.Item.FindControl("FileNameOnlyLbl");
                fileNameOnlyLbl.Text = (new FileInfo(fileNameLbl.Text)).Name;
                fileNameOnlyLbl.Visible = true;

                RadLabel fileNameLbl_Lbl = (RadLabel)e.Item.FindControl("FileNameLbl_Lbl");
                fileNameLbl_Lbl.Visible = true;
            }
            ...
            if (e.Item is GridDataItem)
            {
                ...
                if (item["Notes"].Text.Length > 74)
                {
                    item["Notes"].ToolTip = item["Notes"].Text.ToString(); // Set the Tooltip
                    item["Notes"].Text = (item["Notes"].Text).Substring(0, 75) + "...";
                }
            }
        }
        ...
        protected void ObjectDataSource_Certifications_Deleting(object sender, ObjectDataSourceMethodEventArgs e)
        {
            ...
            RadGrid_Certifications.DataSource = null;
            RadGrid_Certifications.Rebind();
        }
        ...
        protected void RadButton_Save_Click(object sender, EventArgs e)
        {
            ...
            if (PersonId == 0)
            {
                row.PayrollOTRBenefitsStatusId = (Int16)Enums.PayrollBenefitsStatus.NewEmployeeWaitingPeriod;
                row.PayrollOTRPersonLeaveTypeId = (Int16)Enums.PayrollLeaveType.Unassigned;
            ...
            if (bNewPerson)
            {
                ...
                Datasets.dsPersonEdit_DriverInfoTableAdapters.QueriesTableAdapter driverInfoTableAdapt = new Datasets.dsPersonEdit_DriverInfoTableAdapters.QueriesTableAdapter();
                driverInfoTableAdapt.sp_CreateDriverId(PersonId, ref DriverId);
            ...
            Label_UpdateSuccess_Top.Visible = true;
            ScriptManager.RegisterClientScriptBlock(this.Page, typeof(Page), "ShowUpdateSuccess_Top",
                "window.setTimeout(function() { document.getElementById('" + Label_UpdateSuccess_Top.ClientID + "').style.display = 'none' }, 2000);", true);
        }
        ...
        protected void CustomValidator_Name_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            Label_UpdateSuccess_Top.Visible = false; // Making this invisible because it shows up in rare validation scenario.
