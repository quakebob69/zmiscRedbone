...
namespace RedbonePlatform.Pages.ContainerPages
{
	public partial class DispatchSheet : System.Web.UI.Page
	{
			...
			else
			{
				...
				if (CurrentDockStates.Count > 0)
				{
					...
					for (int i = 0; i < CurrentDockStates.Count; i++)
					{
						...
						if (!IsEdiLocked || (!dock.Tag.Contains("pick") && !dock.Tag.Contains("drop")))
						{
		...
		private void CheckIsEdiLoad()
		{
			...
			using (Datasets.dsDispatchTableAdapters.LoadTableAdapter adaptLoad = new Datasets.dsDispatchTableAdapters.LoadTableAdapter())
			{
				Datasets.dsDispatch.LoadRow rowLoad = adaptLoad.GetData(loadId).FirstOrDefault();

				IsEdiLoad = rowLoad != null && rowLoad.TenderId != 0;
		...
		private Datasets.dsLoadStatus.LoadStatusSubTypeDataTable GetSubTypes()
		{
			...
			if (id > 0)
			{
				...
				//tblStats = adapt.GetDataByTypeId("T");
			}
		...
		public void LoadPopulate()
		{
			try
			{
				...
				//Datasets.dsLoadStatus.LoadToLoadStatusSubTypeDataTable tblcomWorkType = comWorkTypeAdapt.GetDataByLoad_Status(LoadId);
				...
				if (collection.Count != 0)
				{
					foreach (RadComboBoxItem item in collection)
					{
						...
						string textVal = item.Text;

						var currObj = tblcomWorkType.Where(x => x.LoadStatusSubTypeId == numVal).FirstOrDefault();
						//var currObj = tblcomWorkType.Where(x => x["Name"].ToString() == textVal).FirstOrDefault();
		...
		private void SaveLoad()
		{
			...
			if (RadComboBox_Status.SelectedIndex != -1)
			{
				...
				if (selectedStatusId != rowLoad.LoadStatusTypeId)
				{
					...
					//status "Hold Billing" (6) and "Awaiting Cust Appr" (9) can have sub-statuses selected
					if (ComWorkTypeItems.Count() > 0 && selectedStatusId != 6 && selectedStatusId != 9)
					{
					...
					rowLoad.ProblemInd = statType == 6 || statType == 9;
			...
			if (ComWorkTypeItems.Count != 0)
			{
				...
				if (!statChanged || selectedStatusId == 6 || selectedStatusId == 9)
				{
		...
		private RadDock CreateRadDock(Datasets.dsDispatch.LoadStopRow rowLoadStop)
		{
			...
			if (!IsEdiLocked || ((Enums.LoadStopType)rowLoadStop.LoadStopTypeId != Enums.LoadStopType.Pick && (Enums.LoadStopType)rowLoadStop.LoadStopTypeId != Enums.LoadStopType.Drop))
			{
				dock.Commands.Add(new DockCloseCommand());
			}
