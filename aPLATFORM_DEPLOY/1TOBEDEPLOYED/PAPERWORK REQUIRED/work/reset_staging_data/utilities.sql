...
--================================
	...
	DECLARE @numberBeforeHistoryRecs INT
		...
		-------
			...
			SELECT COUNT(*) AS 'NUM history RECS - START' FROM [payroll].[PayrollOTRStaging]
		...
		-------	
			...
			SELECT '------------------------------------------'
			SELECT '---------------------STAGE---------------------'
			exec [payroll].[sp_Payroll_OTR_StagePayRecords__DriverPay] @payperiod, 2775
		...
		-------
			...
			SELECT '------------------------------------------'
			SELECT '---------------------FINALIZE---------------------'
			exec [payroll].[sp_Payroll_OTR_FinalizePayPeriod] @payperiod, 2775
