...
CREATE procedure [dbo].[sp_PR_OTRMileageByDateRange____________DELETETHIS____new]
...
BEGIN
	...
	SELECT	
			...
			sum(CURRENT_MILEAGE) AS Quantity
	FROM	#tmp t
			LEFT JOIN #CONCAT c ON c.LoadId = t.LoadId
	WHERE
			(substring(c.DRIVER_1_ID,1,100) = 'KRISTOPHER GARDNERs') and 
			(DELIVER_BY between @BeginDate and @EndDate)
	GROUP BY
			substring(c.DRIVER_1_ID,1,100), CURRENT_MILEAGE
	ORDER BY
			sum(CURRENT_MILEAGE) DESC
